...
import { useClickOutside } from "@/components/reusable/ClickOutsideDiv";
import { modeStyles, languageOptions } from "@/data/layoutData";
import { useTheme } from "@/context/ThemeContext";
import { showToast } from "@/components/reusable/Toasters";
import { HandleResetProps } from "@/types/theme";
            ...
            {showConfig && (
                <div className="fixed z-50 right-0 top-0 w-[400px] h-full
                                ...
                                <div key={index} className="flex flex-col w-1/3 justify-center">
                                    <button
                                        ...
                                        className={`w-full py-3 rounded-[14px] cursor-pointer transition-all duration-200 ${(theme || savedTheme) === mode.type  // Add fallback to savedTheme
                                            ? `bg-linear-to-r ${mode.activeGradient} shadow-md`
                    ...
                    <div id="languageSettings" className="flex flex-col mt-8">
                        <div className="flex items-center justify-center bg-linear-to-r from-green-500 to-green-600 rounded-md h-8 p-2 w-fit text-[13px] text-white font-medium shadow-sm dark:from-green-700 dark:to-green-800">
                                ...
                                <div key={index} className="flex flex-col w-1/3 justify-center">
                                    <button
                                        ...
                                        className={`w-full py-3 rounded-[14px] cursor-pointer transition-all duration-200 ${language === languagearray.type
                                            ? `bg-linear-to-r ${languagearray.activeGradient} shadow-md`
                                            : "bg-gray-300 hover:bg-gray-400 dark:bg-gray-700 dark:hover:bg-gray-600"
                                            }`}
                                    >
